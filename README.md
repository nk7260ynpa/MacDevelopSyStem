...
- 若使用 K8s 方案，需額外具備：
  ...
  - K8s manifests 已採**最小資源設定**（開發取向、非生產規格）：GitLab 與 Harbor 的記憶體
    limits 合計約 6 GB（壓縮前約 14 GB），Docker 分配 4–6 GB 即可嘗試啟動兩者；詳見各工具
    的 K8s 章節。
...
> requests `250m / 1.5Gi`、limits `2 / 3Gi`。屬開發取向、非生產規格。若首次初始化出現
> `OOMKilled`，可將 `02-deployment.yaml` 的 memory limit 調回 `4Gi`。
