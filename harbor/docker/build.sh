...
# 映像版本（含 prepare 共 9 個）集中在 ../versions.env，K8s 方案共用同一份。
# 以 set -a 匯出，docker-compose.yaml 的 ${HARBOR_IMAGE_XXX} 才取得到。
...
echo "[build.sh] 使用 ${HARBOR_IMAGE_PREPARE} 從 harbor.yml 產生設定..."
